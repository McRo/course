...
var points = L.geoCsv (null, {
    ...
    pointToLayer: function (feature, latlng) {
  		return L.marker(latlng, {
  			icon:L.divIcon({ html: '<div>'+/*cluster.getChildCount()+*/'</div>', className: 'my-marker-cluster', iconSize: 20})
  		});
	 },
...
var addCsvMarkers = function() {
    ...
    markers = new L.MarkerClusterGroup({
			...
			iconCreateFunction: function (cluster) {
				return L.divIcon({ html: '<div>'+/*cluster.getChildCount()+*/'</div>', className: 'my-marker-cluster-2', iconSize: 40});
			},
			//Disable all of the defaults:
			spiderfyOnMaxZoom: true, zoomToBoundsOnClick: true, singleMarkerMode: false
		});
